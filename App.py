...
def DoctorHome():
    ...
    # cursor = conn.cursor()
    cur = conn.cursor()
    cur.execute("SELECT * FROM doctortb where username='" + uname + "'")
    data = cur.fetchall()
...
def searchid():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT  *  FROM apptb where id='" + str(user) + "'")
    data = cur.fetchall()
    #print(data)
...
def assigndrug():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO  drugtb VALUES (NULL,'" + uname + "','" + phone + "','" + email + "','" + dname + "','" + medi + "','" + other + "','" + file.filename + "','" + Adate + "')")
        conn.commit()
        conn.close()
...
def Appointment():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO  apptb VALUES (NULL,'" + uname + "','" + mobile + "','" + email + "','" + dusername + "','" + date + "','" + spec + "','"+ dise  +"')")
    conn.commit()
    conn.close()
